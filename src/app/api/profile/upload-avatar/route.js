...
async function uploadToCloudinary(file) {
  ...
  // Use image upload endpoint for profile pictures
...
export async function POST(request) {
  try {
    ...
    // Limit to reasonable size (e.g., 5MB for profile pictures)
    ...
    await userCollection.updateOne(
      ...
      { 
        $set: { 
          image: avatarUrl,
          avatar: avatarUrl, // Also save as avatar for consistency
          updatedAt: new Date() 
        } 
      }, 
      { upsert: true }
