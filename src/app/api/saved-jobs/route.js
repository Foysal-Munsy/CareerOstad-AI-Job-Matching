...
export async function GET() {
  try {
    ...
    const jobs = jobIds.length ? await jobsCollection.find({ _id: { $in: jobIds } }).toArray() : [];

    const jobsMap = new Map(jobs.map(j => [j._id.toString(), j]));
    const result = saved.map(s => ({ ...s, job: jobsMap.get(s.jobId) || null }));
...
export async function POST(request) {
  try {
    ...
    const exists = await savedCollection.findOne({ userEmail: session.user.email, jobId: jobId });
    if (exists) {
